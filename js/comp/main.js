...
define(function (require) {
	...
	function init() {
		...
		topCamera = new THREE.OrthographicCamera(window.innerWidth / -4, // Left
		...
		if (config.postprocess && mode != 'VR') setupPostprocessing();
	...
	function setupLights() {
		...
		var dl = new THREE.DirectionalLight(16777215);
		dl.position.x = 1000;
		dl.position.y = 1000;
		dl.position.z = 750;
		...
		var directionalLight = new THREE.DirectionalLight(8421504);
	...
	function startGame() {
		...
		$('#menu').removeClass('active').addClass('scale-out');
		...
		var at = { r: radious };
		var target = { r: 1600 };
		var zoomT = new TWEEN.Tween(at).to(target, deltaT).easing(TWEEN.Easing.Cubic.Out).onUpdate(function () {
			radious = this.r;
			setCameraPosition();
		}).onComplete(function () {
			game.startLevel(0);
  ...
  key('left,a, down,s, right,d, up,w', function(){
	...
	function onWindowResize(event) {
		...
		//composer.reset();
		if (mode !== 'VR') setupPostprocessing();
	}
